...
if purpose == 'multi':
    ...
    if wanted_procs == '':
        ...
        processes = wanted_procs
        print('Running on %d processes.' % (wanted_procs))
    else:
...
if plot_region == 'CANTERBURY':
    ...
    region_sites = ['Rolleston', 'Darfield', 'Lyttelton', 'Akaroa', 'Kaiapoi', 'Rakaia', 'Oxford']
    dist_scale = '-L172.50/-43.90/$modellat/25.0 -Ba30mf30mWSen'
elif plot_region == 'SOUTHISLAND':
    x_min, x_max, y_min, y_max = 166.0, 174.5, -47.50, -40.00
    region_sites = ['Queenstown', 'Dunedin', 'Tekapo', 'Timaru', 'Christchurch', 'Haast', \
            'Greymouth', 'Westport', 'Kaikoura', 'Nelson', 'Blenheim')
    dist_scale = '-L173/-47/$modellat/50.0 -Ba60mf60mWSen'
elif plot_region == 'MIDNZ':
    x_min, x_max, y_min, y_max = 168.2, 177.9, -45.7, -37.85
    region_sites = ['Queenstown', 'Tekapo', 'Timaru', 'Christchurch', 'Haast', 'Greymouth', \
            'Westport', 'Kaikoura', 'Nelson', 'Blenheim', 'Wellington', 'Palmerston North', \
...
# boundaries/region in GMT format
ll_region = '%s/%s/%s/%s' % (x_min, x_max, y_min, y_max)
# calculate distance on figure (page size) for plotting region
# only image accurate if using a straight lat/lon projection
# inch / degree
ipd = 0.6
x_size = 0, ll_avg[0] * ipd / 2.0, ll_avg[0] * ipd
y_size = 0, ll_avg[1] * ipd / 2.0, ll_avg[1] * ipd
...
# write sim domain corners to file for GMT
# TODO: pass through variable
with open('sim.modelpath', 'w') as pathf:

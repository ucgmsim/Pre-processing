...
def q1(srf_dir):
    print_title("Select Rupture Model - Step 1.")
...
def q2(srf_selected_dir, srf_file_options):
    print_title("Select Rupture Model - Step 2.")
...
def q4(vel_mod_dir):
    print_title("Select one of available CanterburyVelocityModels (from %s)" % (vel_mod_dir))
...
def q6(recipe_dir):
    ...
    print_title("Choose one of available recipes (from %s)" % (recipe_dir))
...
def q7(run_name, recipe_selected_dir):
    print_title("To be created:\n%s\nRecipie ot be copied from\n%s" % \
            (run_name, recipe_selected_dir), hr_char = "*")
...
def action(sim_dir, recipe_selected_dir, run_name, version, global_root, \
        user_root, run_dir, vel_mod_dir, srf_dir, srf_file, stoch_file):

    lf_sim_dir, hf_dir, bb_dir, figures_dir = \
            os.path.join(sim_dir, "LF"), os.path.join(sim_dir, "HF"), \
            os.path.join(sim_dir, "BB"), os.path.join(sim_dir, "Figures")
...
def main():
    print_title(" " * 37 + "EMOD3D Job Preparationi Ver." + bin_process_ver, hr_char = "*")
    ...
    action(sim_dir, recipe_selected_dir, run_name, emod3d_version, global_root, \
            user_root, run_dir, vel_mod_dir_full, srf_dir, srf_file, stoch_file)

    # add bin_process to PATH if it is not already there
    if not bin_process_dir in os.environ['PATH']:
        f = open("/home/%s/.bashrc" % (user), 'a')
        # stable instead of current version to avoid PATH clutter
        f.write("export PATH=$PATH:%s\n" % (os.path.join(bin_process_path, "stable")))

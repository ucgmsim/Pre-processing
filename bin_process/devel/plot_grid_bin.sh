...
makecpt -Chot -I -T0/80/1 -A50 > $base_cpt
...
psxy -JX8.5/11 -R0/8.5/0/11 -L -G${edge_colour} -X0 -Y0 -K << END > "$plot_file" #-W0/180/180/180
0.3 1.0
0.3 7.8
6.5 7.8
6.5 1.0
END
# set the color scale
psscale -C$base_cpt -Ef -D3.0/2.0/2.5/0.15h -K -O -Ba10f$10:"ground velocity (cm/s)": >> "$plot_file"
# specify the X and Y offsets for plotting (I dont really understand this yet)
psxy -V $att -L  -K -O -X$plot_x_org -Y$plot_y_org << END >> "$plot_file" 2>/dev/null #-W5/255/255/0
END
# try a different version of plotting
# clippath for land
pscoast $att -Df -Gc -K -O >> "$plot_file"
# land
grdimage $plot_topo_file $plot_topo_illu $plot_palette $att -K -O >> "$plot_file"
# clear clippath
pscoast -R -J -O -K -Q >> "$plot_file"
# add urban areas
URBANDIR=${global_root}/PlottingData/sourcesAndStrongMotionStations
psxy ${URBANDIR}/ChchUrbanBoundary.xy $att -G160/160/160 -W0.5p -O -K >> "$plot_file"

# create ground motion intensity surface
surface $input -Gmotion.surface -I$plot_dx/$plot_dy \
        -R$plot_ts_region -T0.0 -bi3f
# clip minimum (prevents scattered spots)
grdclip motion.surface -Gmotion.surface \
        -Sb${plot_topo_a_min}/$plot_topo_a_below
# add surface as image to plot
grdimage motion.surface $att -C$base_cpt -Q -t50 -K -O >> "$plot_file"

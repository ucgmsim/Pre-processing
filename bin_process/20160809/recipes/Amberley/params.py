...
# sim_duration: max simulation timeperiod = nt * dt eg: 10,000 * 0.005 = 50 seconds
sim_duration = '200'
# dt should be 0.005 (or smaller), small increments required in simulation
dt = '0.005'
nt = str(int(float(sim_duration)/float(dt))) #'40000' #'20000'
...
ts_start = '0'
ts_total = str(int(sim_duration)/(float(dt)*float(dt_ts)))
...
vel_mod_params_dir = os.path.join(global_root, 'VelocityModel/Amberley')
...
# files
stat_file = os.path.join(stat_dir, 'geonet_stations_201611.ll')
stat_coords = os.path.join(stat_dir, 'fd_amb01-h0.400.statcords')
...
GRIDFILE = os.path.join(vel_mod_params_dir, 'gridout_amb01-h') + hh# GRIDFILE is the file containing the local (x,y,z) coordinates for this 3D run
...
plot_main_title = 'Mw7.8 Amberley 400m Earthquake'
plot_sub_title = 'South-North ver1.0'
...
MODELPARAMS = os.path.join(vel_mod_params_dir, 'model_params_amb01-h') + hh
...
MODEL_COORDS = os.path.join(vel_mod_params_dir, 'model_coords_amb01-h%s' % (hh))

...
class siteProperties:  # Class of site properties. initialize all attributes to None
    period = (
        None  # '(-1),(0),(real variable)' period of vibration =-1->PGV; =0->PGA; >0->SA
    )
    ...
    V30measured = (
        None  # yes =1 (i.e. from Vs tests); no=0 (i.e. estimated from geology)
    )
...
def calculateGMPE(
    parms, ImName, period
):  ##need to pass the ImName and the period as we loop over these
    # Do some error checking to make sure that the model is valid

    if parms.plotGMPE.model != 1:
        print(
            "Error: The selected GMPE (model %d) is not supported. Only Model=1 Bradley_2010 is supported."
        ) % (parms.plotGMPE.model)
    ...
    if not ((ImName == "PGA") | (ImName == "PGV") | (ImName == "pSA")):
        print("Error: The selected GMPE (model %d) does not support IM name %s") % (
            parms.plotGMPE.model,
            ImName,
        )
    ...
    Rrupval = np.exp(
        np.linspace(
            np.log(parms.plotGMPE.DistMin),
            np.log(parms.plotGMPE.DistMax),
            parms.plotGMPE.nVal,
        )
    )
    ...
    for i in range(len(parms.plotGMPE.SiteClassForPrediction)):
        if parms.plotGMPE.SiteClassForPrediction[i] == "A":
            ...
            print(
                parms.plotGMPE.SiteClassForPrediction[i],
                "is an invalid siteClass for Prediciton. Must be A, B, C, D, E.",
            )
    ...
    for j in range(len(Vs30)):  # loop over the different Vs30 values chosen
        ...
        for i in range(len(Rrupval)):  # loop over the different rupture distances
            ...
            if (
                parms.plotGMPE.model == 1
            ):  # Bradley2010 NZ-specific. No error checking here as we check at start of file
                IM_Rscaling[j, i], sigma_IM_Rscaling[j, i, :] = Bradley_2010_Sa(
                    siteprop, faultprop
                )

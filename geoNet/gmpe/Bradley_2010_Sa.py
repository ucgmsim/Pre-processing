    ...
    if np.abs(closestPeriod - T) > tol:  # interpolate between periods if neccesary
        ...
        if T_low > 0:  # from .m >eps
            ...
            for i in range(len(sigma_SA_low)):
                sigma_SA.append(
                    np.interp(np.log(T), x, [sigma_SA_low[i], sigma_SA_high[i]])
        ...
        if T == 0:
            ...
            v1 = np.min((np.max((1130.0 * (T / 0.75) ** (-0.11), 1130.0)), 1800.0))
        ...
        tau = tau1[i] + (tau2[i] - tau1[i]) / 2.0 * (
            np.min((np.max((M, 5.0)), 7.0)) - 5.0
        )

        # outputs
        SA = Sa
        sigma_SA = []
        sigma_SA.append(np.sqrt((1.0 + NL0) ** 2 * tau**2 + sigma**2))  # 0
        sigma_SA.append((1.0 + NL0) * tau)  # 1

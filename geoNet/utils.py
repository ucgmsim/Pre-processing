...
def get_PSA(acc_dict, dt, periods, xi=0.05, m=1, gamma=0.5, beta=0.25):
    """
...
def get_event_PSA(event_acc, period, xi=0.05, m=1.0, gamma=0.5, beta=0.25):
    """
...
def get_sorted_stats_code(loc, stats_dict, comp="geom"):
    ...
    for stat_code in stats_dict.keys():
        ...
        sorted_stats_code.append(
            {"000": max_000, "090": max_090, "geom": max_geom, "name": stat_code}
        )
...
def get_empIM_v2(
    Rrup, period, faultprop, Rjb=None, Rtvz=0.0, V30measured=0.0, V30=250.0
):
    """
    ...
    if Rjb is None:
        Rjb = -Rrup
    V30 = np.asarray(V30)
    if not Rrup.size == V30.size:
        raise AssertionError("Rrup.size not equal to V30.size")
    ...
    for i, (x, y, z) in enumerate(zip(Rrup, Rjb, V30)):
        # x,y = Rrup[i], Rjb[i]
        siteprop = set_siteprop(
            faultprop,
            period=np.nan,
            Rrup=x,
            Rjb=y,
            Rtvz=Rtvz,
            V30measured=V30measured,
            V30=z,
        )
...
def get_processed_stats_list(loc, stats_dict, verbose=False):
    ...
    for stat_code in psc:
        if stats_dict.has_key(stat_code):
            processed_stats_dict.update({stat_code: stats_dict[stat_code]})
        else:
...
def parse_V1A_fname(fname):
    ...
    if fname.count("_") == form1.count("_"):
        ...
        raise Exception(
            "{:s} is unknow file name format for .V1A files\n".format(fname)
        )

    return YYYYMMDD, HHMM, stat_code


def statsll_from_V1A(loc_V1A, save_stats=False, fname=None, loc=os.getcwd()):
    """
    ...
    for V1A_file in event_stats_V1A:
        try:
            event_stats.update(statcode_lonlat_from_V1A(loc_V1A, V1A_file))
        except Exception as e:
            print(e)
...
def statsll_from_V1A_v2(
    ...
    if len(stats_not_in_all_geoNet_stats) > 0:
        print(
            "(lon, lat) for the following stations were not in {:s}:\n".format(
                fname_all_geoNet_stats
            )
        )
...
def get_SMS_pSA(
    stat_codes, period, loc, comp="geom", xi=0.05, m=1.0, gamma=0.5, beta=0.25
):
    """
    ...
    for i, stat_data in enumerate(event_data):
        ...
        pSA = get_PSA(stat_data, dt, period, xi=xi, m=m, gamma=gamma, beta=beta)
        SMS_pSA[i, :] = pSA[comp]
...
def int_stat_data(stat_data):
    ...
    for key in ["000", "090", "ver"]:
        int_stat_data[key] = cumtrapz(
            y=stat_data[key], dx=stat_data["t"][1] - stat_data["t"][0], initial=0.0
        )
    return int_stat_data
...
def filt_stat_data(stat_data, freq, btype, output="sos", order=4, worN=512):
    """
    Note:

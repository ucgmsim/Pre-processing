...
def get_stat_acc_plot(loc, stat_code):
    ...
    fig.suptitle(
        stat_code
        + ", PGA geometric mean="
        + "{:.4f}".format(np.sqrt(np.abs(ext_000 * ext_090)))
    )
    return stat, fig, ax
...
def get_stat_vel_plot(loc, stat_code):
    ...
    ax[0].plot(stat["t"][argext_000], ext_000, marker="o")
    ax[0].annotate(
        "{:8.2f}".format(ext_000), xy=(stat["t"][argext_000], stat["000"][argext_000])
    )
    ...
    ax[1].plot(stat["t"][argext_090], ext_090, marker="o")
    ax[1].annotate(
        "{:8.2f}".format(ext_090), xy=(stat["t"][argext_090], stat["090"][argext_090])
    )
    ...
    fig.suptitle(
        stat_code
        + ", PGV geometric mean="
        + "{:.2f}".format(np.sqrt(np.abs(ext_000 * ext_090)))
    )
    return stat, fig, ax


def get_stat_PSA_plot(
    loc, stat_code, period=np.logspace(np.log10(0.01), np.log10(10.0), 100)
):
    """ """
    # stat=get_stat_data(loc,stat_code)
    stat = get_adjusted_stat_data(loc, stat_code)
...
def plot_bias(periods, bias, std, savefig=True, figName="bias", ext="png"):
    ...
    ax_bias.grid(
        b=True, axis="y", which="major"
    )  # turn the major grid lines on for y axis
...
def plot_SMS_IM(Rrup, IM_sim, IM_obs):
    ...
    """
    fig, ax = plt.subplots()
    ax.scatter(Rrup, IM_obs, c="g", linewidths=2.0, marker="+", s=40, label="Obs")

...
def plot_all(event_info_fname):
    ...
    for i, stat_code in enumerate(sorted_stats_code):
        fig, ax = plt.subplots()
        ax.loglog(
            periods, pSA_obs[i, :], c="k", lw="1", ls="solid", label=stat_code + " obs"
        )
        ax.legend(loc="best")
    ...
    fig, ax = plt.subplots()
    ax.scatter(
        sms_rrup, PGV_obs[:, -1], c="g", linewidths=2.0, marker="+", s=40, label="Obs"
    )
    ax.set_xscale("log")
    ax.set_yscale("log")
    # Now underlay gmpe predictions
    ...
    fig, ax = plt.subplots()
    ax.scatter(
        sms_rrup, PGA_obs[:, -1], c="g", linewidths=2.0, marker="+", s=40, label="Obs"
    )
    ax.set_xscale("log")
    ax.set_yscale("log")
    # Now underlay gmpe predictions

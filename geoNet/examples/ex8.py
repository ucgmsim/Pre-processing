...
def plot_all(event_info_fname):
    ...
    stats_dict_sim = utils.read_statsll(
        info["loc_statsll_sim"], info["fname_statsll_sim"]
    )
    ...
    stats_dict_obs = utils.get_processed_stats_list(
        "/".join([info["loc_V1A"], info["obs_velDir"]]), stats_dict_obs, verbose=True
    )

    stats_dict_simObs = {
        key: stats_dict_sim[key] for key in (set(stats_dict_sim) & set(stats_dict_obs))
    }
    FiniteFault = rsf.readSrfFile("/".join([info["srf_dir"], info["srf_file"]]))
    FiniteFault = rsf.Points_np(FiniteFault)
    # set fault properties
    faultprop = set_faultprop(
        Mw=float(info["Mw"]),
    ...
    sorted_stats_code, sms_rrup_rjb = utils.get_SMS_Rrups(
        stats_dict_simObs, FiniteFault
    )
    ...
    figBias, axBias = putils.plot_bias(
        periods, bias, std, savefig=True, figName="figs/bias", ext="png"
    )
    ...
    for i, stat_code in enumerate(sorted_stats_code):
        ...
        ax.loglog(
            periods, pSA_sim[i, :], c="r", lw="1", ls="solid", label=stat_code + "\nsim"
        )
    ...
    PGV_sim = utils.get_SMS_PGV(
        sorted_stats_code, "/".join([info["bb_dir"], info["sim_velDir"]]), absVal=False
    )
    PGV_obs = utils.get_SMS_PGV(
        sorted_stats_code, "/".join([info["loc_V1A"], info["obs_velDir"]]), absVal=False
    )
    ...
    PGA_obs = utils.get_SMS_PGA(
        sorted_stats_code, "/".join([info["loc_V1A"], info["obs_accDir"]]), absVal=False
    )
    ...
    # Now underlay gmpe predictions
    fig, ax = putils.plot_IMvsRrup(
        Rrups_gmpe, PGV_gmpe[:, 0], PGV_gmpe_std[:, 0], fig=fig, ax=ax
    )
    ax.legend(loc="best", scatterpoints=1)
    ...
    # Now underlay gmpe predictions
    fig, ax = putils.plot_IMvsRrup(
        Rrups_gmpe, PGA_gmpe[:, 0], PGA_gmpe_std[:, 0], fig=fig, ax=ax
    )
    ax.legend(loc="best", scatterpoints=1)

...
def butter_bandpass(lowcut, highcut, fs, order=5):
    ...
    low = lowcut / nyq
    high = highcut / nyq
    sos = fd.butter(order, [low, high], btype='band', analog=False,\
            output='sos')
    return sos
...
class ButterWorth:
    def __init__(self, data, fs=(1./0.005), ft=1., order=4):
        self.data = data
        self.fs   = fs
        self.ft   = ft
        self.order= order
    ...
    @staticmethod
    def filter(data, btype='lowpass', fs=(1./0.005), ft=1., order=4, 
                        output=None):
        """
        btype:
            'lowpass' or 'highpass'
        ...
        Nyquist_fq = 0.5*fs
        Wn = ft/Nyquist_fq
        sos = fd.butter(order, Wn, btype=btype, analog=False,
                        output='sos')
    ...
    def plot_freqResp(self, worN=512):
        ...
        self.y_lf, self.sos_lf = self.filter(self.data, 'lowpass',
                                 self.fs, self.ft, self.order, 'sos')
        self.b_lf, self.a_lf = fd.sos2tf(self.sos_lf)
        w_lf, h_lf = freqz(self.b_lf, self.a_lf, worN=worN) #default worN=512)
        
        self.y_hf, self.sos_hf = self.filter(self.data, 'highpass', 
                                 self.fs, self.ft, self.order, 'sos')
        self.b_hf, self.a_hf = fd.sos2tf(self.sos_hf)
        w_hf, h_hf = freqz(self.b_hf, self.a_hf, worN=worN) #default worN=512)

        w_lf, h_lf, w_hf, h_hf = (w_lf*0.5*self.fs*0.5/np.pi, np.abs(h_lf),
                                  w_hf*0.5*self.fs*0.5/np.pi, np.abs(h_hf))
        plt.figure()
        ...
        fig = plt.figure()
        t = np.linspace(0., self.data.size*0.005, self.data.size, endpoint=False)
...
if __name__ == "__main__":
    ...
    for order in [3, 6, 9]:
        ...
        low = lowcut / nyq
        high = highcut / nyq
        b, a = fd.butter(order, [low, high], btype='band', analog=False,\
            output='ba')

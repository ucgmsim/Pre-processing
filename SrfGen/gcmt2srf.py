...
uncertainty_group = parser.add_argument_group(
    "Uncertainty options",
    "Options to be used when SRFs are to be genreated " "with uncertainty factors",
)
...
if all_opts:
    ...
    cs_file_pd = cs_file_pd.transform(
        {"pid": lambda x: x, "N_Rels": lambda x: int(x[:-1]) if x.endswith("r") else 0}
    )
...
depth_bins = np.digitize(
    np.around(sources.depth, decimals=5), np.around(np.cumsum(vmodel.depth), decimals=5)
)

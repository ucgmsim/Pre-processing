...
def run_create_srf(args, t, vs, rho, n_sims, logger: Logger = qclogging.get_basic_logger()):
    ...
    try:
        ...
        pid = t.pid
    logger.debug("Entered run_create_srf. pid is {}. Switching to fault logger".format(pid))
    ...
    if args.uncertainty_file:
        fault_logger.debug("Uncertainty file found. Generating perturbated realisations")
        ...
        create_ps_realisation(args.out_dir, pid, t.lat, t.lon, t.depth, t.mag, mom, t.strike, t.rake, t.dip,
                              ...
                              silent=True, logger=fault_logger)
        ...
        gen_meta(
            ...
            file_name=os.path.join(args.out_dir, pid, pid), logger=fault_logger
        )
    else:
        ...
        CreateSRF_ps(t.lat, t.lon, t.depth, t.mag, mom, t.strike, t.rake, t.dip, dt=args.dt, prefix=prefix, stoch=stoch,
                     vs=vs, rho=rho, silent=True, logger=fault_logger)
    if args.plot:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--out-dir', help='directory to place outputs', default='./autosrf')
    ...
    uncertainty_group = parser.add_argument_group(
        'Uncertainty options', 'Options to be used when SRFs are to be generated with uncertainty factors'
    )
    ...
    # validate parameters
    qclogging.add_general_file_handler(logger, os.path.join(args.out_dir, 'gcmt2srf_log.txt'))

    error_messages = []
    if not os.path.exists(args.csv_file):
        error_messages.append('CMT solutions CSV file not found: {}'.format(args.csv_file))

    if not os.path.exists(args.velocity_model):
        error_messages.append('Velocity model file not found: {}'.format(args.velocity_model))
    ...
    # load CMT CSV
    sources = np.rec.array(np.loadtxt(
        args.csv_file,
        dtype=[
            ('pid', '|S32'), ('lat', 'f8'), ('lon', 'f8'), ('depth', 'f8'),
            ('mag', 'f8'), ('strike', 'f8'), ('dip', 'f8'), ('rake', 'f8')
        ],
        delimiter=',',
        skiprows=1, usecols=(0, 2, 3, 13, 11, 4, 5, 6)
    ))
    ...
    if all_opts:
        ...
        logger.debug("No cybershake file given, creating one realisation for each source")
        n_sims = np.ones(len(sources))

    # load velocity model (vs and rho)
    vmodel = np.rec.array(
        np.loadtxt(
            args.velocity_model, skiprows=1, usecols=(0, 2, 3), dtype=[('depth', 'f8'), ('vs', 'f8'), ('rho', 'f8')]
        )
    )

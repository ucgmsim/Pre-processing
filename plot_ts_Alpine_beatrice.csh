...
set MAIN_TITLE = "Mw7.9 Alpine Fault test case"
set SUB_TITLE = "SthHypo, Stoch Slip, SIVM_v1.01_h0.4km"

#name of the simulation run
set NAME = AlpineM7.9SthHypo_v1_SthIslandv1.01-h0.400_v3.04

#High-level directories
#set MAINDIR = /Users/bbradley/Documents/Engineering/Research/ResearchTopics/GroundMotion/GmSimulation/CantGmSimulation
#set SIMDIR = ${MAINDIR}/SimulatedGroundMotions/AlpineFaultScenarioTest
set MAINDIR = ~bab70
set SIMDIR = ../
...
#set TSBIN = /Users/bbradley/Documents/Engineering/Research/ResearchTopics/GroundMotion/GmSimulation/CantGmSimulation/GravesCodesScripts/Bin
set TSBIN = ${MAINDIR}/Bin
#location of the TSlice binary file created by 'merge_tsP3.csh'
set TSFILE = ${SIMDIR}/OutBin/${NAME}_xyts.e3d
#
...
set TSFILEDIR = ${SIMDIR}/TSlice/TSFiles

#details of the spatial and termporal discretization and spacing
set HH = 0.400 #spatial grid spacing in km
...
set VELMODPARAMSDIR = ${MAINDIR}/VelocityModel/SthIsland/ModelParams
set GRIDFILE = ${VELMODPARAMSDIR}/gridout_sinz01-h${HH}   #used in 'ts2xyz' exe - so not presentely used
set MODELPARAMS = ${VELMODPARAMSDIR}/model_params_sinz01-h${HH} #used to get location of model corners

#specify whether or not to byte-swap (=0 no; =1 yes - which should be used if TSFILE created on supercomp and this file is run on laptop) #the three lines below not used if the TSFiles are created using 'gen_ts.csh' on supercomputer and copied to local computer beforehand.
#set SWAP_BYTES = 1
set SWAP_BYTES = 0
set LONLAT_OUT = 1
set SCALE = 1.0
...
set TS_START = 2100    #this is almost always zero
set TS_INC = 1
set TS_TOTAL = 2101 #400    #this will make the total sim time = TS_TOTAL*ORIG_DT
...
#set TOPODIR = /Users/bbradley/Documents/Engineering/Research/ResearchTopics/VelocityModelling/CanterburyVelocityModel/Plotting/TopoData/
set TOPODIR = ${MAINDIR}/PlottingData/TopoData
set TOPO_FILE = ${TOPODIR}/nztopo.grd
set ILLU = -I${TOPODIR}/nztopo_i5.grd
set TOPO_FILE2 = ${TOPODIR}/etopo2.grd
#set PALETTE=-Crelief.cpt
set PALETTE=-Cgray

#specific locations to display on figure (not currently used)
set SITES = ( "Queenstown" "Dunedin" "Tekapo" "Timaru" "Christchurch" "Haast" "Greymouth" "Westport" "Kaikoura" "Nelson" "Blenheim" )
#set SPOS = ( 1 1 1 1 1 1 1)
set SPOS  = ( "LT" "LM" "LM" "LM" "LM" "RM" "RM" "RM" "LM" "CB" "LM" )  #alignment; 2letter, L,C,R (left, center, right); T,M,B (top, middle, bottom)
set SLON  = ( 168.6680556 170.3794444 170.4794444 171.2430556 172.6347222 169.0405556 171.2063889 171.5997222 173.6802778 173.2838889 173.9569444 )
set SLAT  = ( -45.0300000 -45.8644444 -44.0069444 -44.3958333 -43.5313888 -43.8808333 -42.4502777 -41.7575000 -42.4038888 -41.2761111 -41.5138888 )
...
set TS_XMIN = 166.0 #167.0
set TS_XMAX = 174.5 #170.0
set TS_YMIN = -47.50 #-46.00
set TS_YMAX = -40.00 #-45.00
#region in GMT format
...
set AABOVE = `echo $AMAX 0.95 | gawk '{printf "%f\n",$1*$2;}'` #usually 98% best (as surface is tension-fit so using 100% gives some white lines)
...
set STATFILE = ${MAINDIR}/StationInfo/southislandstations_v1.ll
...
while ( $tsfcnt < $TS_TOTAL )
    ...
    set PLOTFILE = $PSDIR/ts-str${tsfnum}.ps
...
END
    ...
    #set the color scale
gmt psscale -C$BASECPT -D3.6/2.0/4.0/0.15h -K -O -Ba${AINC}f${AINC}:"ground velocity (cm/s)": >> $PLOTFILE #-Ef
...
#currently components are not looped over, as using ABSMAX=1
    ...
    foreach comp ( $COMPS )
        ...
        #add urban areas
#set URBANDIR = /Users/bbradley/Documents/Engineering/Research/ResearchTopics/GroundMotion/GmSimulation/CantGmSimulation/plotting/sourcesAndStrongMotionStations
        ...
        gmt grdclip tmp1.grd -Goutf_${comp}.grd -Sb${AMIN}/$ABELOW -Sa${AMAX}/$AABOVE #clip minimum
...
#        gmt pscoast -A0/0/1 -N1 -N2 $ATT -Df -S135/205/250 -W1,black -K -O >> $PLOTFILE
        gmt pscoast -A0/0/1 -N1 -N2 $ATT -Df -W1,black -K -O >> $PLOTFILE
        gmt pscoast -A0/2/2 $ATT -Df -W1,black -K -O >> $PLOTFILE

        #Add fault planes
#set ADDFLTPLANEDIR = /Users/bbradley/Documents/Engineering/Research/ResearchTopics/GroundMotion/GmSimulation/CantGmSimulation/plotting/sourcesAndStrongMotionStations
        set ADDFLTPLANEDIR = ${MAINDIR}/PlottingData/sourcesAndStrongMotionStations
        set FAULTFILE = ${ADDFLTPLANEDIR}/alpineCnrs.txt;
        set LINE = -W0.5p,black,-
        set TOPEDGE = -W2p,black
        set HYPOPEN = -W1p,black;
        #call fault plane routine
        bash ${ADDFLTPLANEDIR}/addStandardFaultPlane.sh $PLOTFILE $ATT $FAULTFILE $LINE $TOPEDGE $HYPOPEN
...
END
        ...
        #now add the Sth Islan vel model domain boundary
        gmt psxy ${ADDFLTPLANEDIR}/SthIslandVelModelDomain.xy $ATT -W0.5p -L -O -K >> $PLOTFILE


        #add scale to show distance
        gmt psbasemap $ATT -L173/-47/$AVGLL[2]/50.0 -Ba60mf60mWSen -K -O >> $PLOTFILE

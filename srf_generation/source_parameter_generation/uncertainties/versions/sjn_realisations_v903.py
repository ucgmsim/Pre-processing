...
def generate_source_params(
    ...
    **kwargs,                                                               #new line  
    ...
    if isinstance(source_data, GCMT_Source):
        realisation["params"] = generate_from_gcmt(source_data, qs_z, additional_source_parameters)    #modified
    else:
    ...
    #HF 1d profile
    HF_onedprofile_ori = pd.read_csv("/nesi/project/nesi00213/VelocityModel/Mod-1D/Cant1D_v3-midQ_OneRay.1d",delim_whitespace=True, names=['depth', 'vp', 'vs', 'rho', 'qp', 'qs'],skiprows=1)
    ...
    total_vs30_uncertainty = np.sqrt((vs30_data["sigma"].values)**2 + vs30_model_uncertainty**2 + measurement_uncertainty**2)

    
    if vs30_data is not None:
        print("Got vs30")
        realisation["vs30"] = vs30_data.copy(deep=True)
        realisation["vs30"]["vs30"] = distributions.truncated_log_normal(vs30_data["median"].values, total_vs30_uncertainty, 2)
    else:
...
def uniform_dist(u_mean, u_half_range):
    ...
    u_sample = np.random.uniform(low=u_mean-u_half_range, high=u_mean+u_half_range, size=1)
    return u_sample



def generate_from_gcmt(sources_line: GCMT_Source, qs_z, additional_source_parameters: Dict[str, Any]):   #modified
    ...
    z_lat_pert = geo.ll_dist(sources_line.lon, sources_line.lat, sources_line.lon, lat)/1.0
    z_long_pert = geo.ll_dist(sources_line.lon, sources_line.lat, lon, sources_line.lat)/1.0

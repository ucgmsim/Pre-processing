...
def generate_from_gcmt(
    ...
    mag = np.random.normal(sources_line.mag, 0.8, 1)  # magnitude
    ...
    sdrop = distributions.truncated_log_normal(50, 0.4, 4)
    rvfac = np.random.normal(0.8, 0.8, 1)  # rupture velocity factor
    if rvfac == 0:
        rvfac = 0.001
    # kappa = distributions.truncated_log_normal(0.045, 0.3, 2)
    kappa = 0.045

    dpath_pert = np.random.normal(0, 0.5, 1)
